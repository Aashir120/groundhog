...
The prompts Groundhog passes to the coding agent live in `prompts/`:

* `prompts/analysis.md` — the one-off data analysis run for a project.
* `prompts/experiment.md` — each experiment loop.

They used to live in this file, below the Reflex-managed block. They were moved
out because `reflex init` regenerates this file, which silently wiped them.
Everything above this line is developer documentation and is not sent to the
agent.
